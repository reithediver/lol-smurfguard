...
export class ReportGenerator {
   ...
   */
  generateCSV(analyses: SmurfAnalysisResult[]): string {
    const headers = [
      'Summoner Name',
      'Smurf Probability (%)',
      'Risk Level',
   ...
   */
  generateJSON(analyses: SmurfAnalysisResult[]): string {
   ...
   */
  generateTournamentSummary(analyses: SmurfAnalysisResult[]): string {
...
=== HIGH RISK PLAYERS ===
${highRisk.map(player => 
  `${player.summonerName}: ${(player.probability * 100).toFixed(1)}% (${player.riskLevel})`
).join('\n')}

=== MODERATE RISK PLAYERS ===
${moderateRisk.map(player => 
  `${player.summonerName}: ${(player.probability * 100).toFixed(1)}% (${player.riskLevel})`
).join('\n')}
...
Report generated by League of Legends Smurf Detection System v1.0.0
   ...
   */
  sendCSVDownload(res: Response, analyses: SmurfAnalysisResult[], filename: string = 'smurf-analysis-report') {
   ...
   */
  sendJSONDownload(res: Response, analyses: SmurfAnalysisResult[], filename: string = 'smurf-analysis-report') {
   ...
   */
  sendTournamentSummaryDownload(res: Response, analyses: SmurfAnalysisResult[], filename: string = 'tournament-summary') {
   ...
   */
  generateStatistics(analyses: SmurfAnalysisResult[]) {
    ...
    analyses.forEach(analysis => {
      if (analysis.championPerformance.overallPerformanceScore > 0.5) championFactorTriggered++;
      if (analysis.summonerSpellUsage.patternChangeScore > 0.3) spellFactorTriggered++;
      if (analysis.playtimeGaps.totalGapScore > 0.3) gapFactorTriggered++;
    });
...
export type { SmurfAnalysisResult }; 