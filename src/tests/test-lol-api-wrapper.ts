...
async function testApiAccess() {
  ...
  try {
    ...
    const statusData = await lolAPI.status.platformData();
  ...
  try {
    ...
    const rotationData = await lolAPI.champion.rotation();
    logger.info('✅ Champion rotation endpoint accessible!');
    if (rotationData.freeChampionIds) {
      logger.info(`Free champion count: ${rotationData.freeChampionIds.length}`);
    } else {
  ...
  try {
    ...
    const leagueData = await lolAPI.league.getChallengerLeague('na1', 'RANKED_SOLO_5x5');
  ...
  try {
    ...
    const summonerData = await lolAPI.summoner.getByName('Doublelift', 'na1');
    ...
    if (summonerData.puuid) {
      try {
        ...
        const matchIds = await lolAPI.match.getIdsByPUUID(summonerData.puuid, 'americas');
