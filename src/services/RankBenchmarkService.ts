...
export interface PlayerComparison {
  ...
  rankAverage: number;
  percentile: number;
  deviation: number; // How many standard deviations from average
  status: 'far_below' | 'below_average' | 'average' | 'above_average' | 'exceptional';
  suspiciousLevel: number; // 0-100, higher = more suspicious
}
...
export interface PlaystyleChange {
  ...
  beforeValue: number;
  afterValue: number;
  significance: number; // Statistical significance of the change
  suspicionScore: number;
  description: string;
}

export class RankBenchmarkService {
  private benchmarks: Map<string, RankBenchmarks[]> = new Map();
  ...
  private initializeBenchmarks() {
    ...
    const benchmarkData: RankBenchmarks[] = [
      // Top Lane
  ...
  getBenchmarkForRank(role: string, rank: string): RankBenchmarks | null {
    const roleBenchmarks = this.benchmarks.get(role);
    if (!roleBenchmarks) return null;
  ...
  comparePlayerToRank(
    ...
    if (playerMetrics.csPerMin !== undefined) {
      comparisons.push(this.createComparison(
        'CS per Minute',
        playerMetrics.csPerMin,
        benchmark.csPerMin,
        this.calculatePercentile(playerMetrics.csPerMin, benchmark.csPerMin, 0.8) // Standard deviation estimate
      ));
    }
    ...
    if (playerMetrics.kda !== undefined) {
      comparisons.push(this.createComparison(
        ...
        playerMetrics.kda,
        benchmark.kda,
        this.calculatePercentile(playerMetrics.kda, benchmark.kda, 0.6)
      ));
    }
    ...
    if (playerMetrics.killParticipation !== undefined) {
      comparisons.push(this.createComparison(
        'Kill Participation',
        playerMetrics.killParticipation,
        benchmark.killParticipation,
        this.calculatePercentile(playerMetrics.killParticipation, benchmark.killParticipation, 8)
      ));
    }
    ...
    if (playerMetrics.visionScore !== undefined) {
      comparisons.push(this.createComparison(
        'Vision Score',
        playerMetrics.visionScore,
        benchmark.visionScore,
        this.calculatePercentile(playerMetrics.visionScore, benchmark.visionScore, 5)
      ));
    }
  ...
  private createComparison(
    ...
    let status: PlayerComparison['status'];
  ...
  calculateOverallSuspicion(comparisons: PlayerComparison[]): number {
