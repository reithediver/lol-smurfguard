export interface TimeWindow {
  ...
  metrics: {
    ...
    championDiversity: number; // How many different champions played
    performanceConsistency: number; // How consistent the performance is
  };
}

export interface ChampionEvolution {
  championName: string;
  timeWindows: {
    period: string;
    games: number;
  ...
  suspicionFlags: {
    ...
    suddenExpertise: boolean; // Went from never playing to expert level
    metaShift: boolean; // Switched to meta champions suddenly
    complexityJump: boolean; // Jumped from simple to complex champions
  };
}

export interface PlaystyleShift {
  timestamp: Date;
  type: 'gradual' | 'sudden' | 'dramatic';
  confidence: number; // Statistical confidence in the shift
  description: string;
  beforePeriod: TimeWindow;
  afterPeriod: TimeWindow;
  metrics: {
...
export class PlaystyleAnalysisService {
  private static readonly WINDOW_SIZE_DAYS = 30; // Analyze in 30-day windows
  private static readonly MIN_GAMES_PER_WINDOW = 10;
  private static readonly SIGNIFICANT_CHANGE_THRESHOLD = 2.0; // Standard deviations
   ...
   */
  async analyzePlaystyleEvolution(matches: any[]): Promise<{
    ...
    const sortedMatches = matches.sort((a, b) => 
      new Date(b.info.gameCreation).getTime() - new Date(a.info.gameCreation).getTime()
    );
    ...
    const overallSuspicionScore = this.calculateOverallSuspicion(
      playstyleShifts, 
      championEvolution, 
      timeWindows
    );
  ...
  private createTimeWindows(matches: any[]): TimeWindow[] {
  ...
  private calculateWindowMetrics(matches: any[]): TimeWindow['metrics'] {
    if (matches.length === 0) {
      return {
        csPerMin: 0,
        kda: 0,
  ...
  private extractPlayerStats(match: any): any {
  ...
  private detectPlaystyleShifts(timeWindows: TimeWindow[]): PlaystyleShift[] {
    if (timeWindows.length < 2) return [];

    const shifts: PlaystyleShift[] = [];
  ...
  private analyzeWindowTransition(before: TimeWindow, after: TimeWindow): PlaystyleShift | null {
    ...
    const complexityIncrease = this.calculateComplexityChange(
      before.metrics.mostPlayedChampions,
      after.metrics.mostPlayedChampions
    );
    ...
    const description = this.generateShiftDescription(
      shiftType,
      csImprovement,
      kdaImprovement,
      visionImprovement,
  ...
  private calculateImprovement(before: number, after: number): number {
    if (before === 0) return after > 0 ? 5 : 0; // Significant if going from 0 to positive
    return (after - before) / before;
  }

  private calculateComplexityChange(beforeChampions: string[], afterChampions: string[]): number {
  ...
  private generateShiftDescription(
    ...
    if (csImprovement > 0.3) improvements.push(`CS improved by ${(csImprovement * 100).toFixed(1)}%`);
    if (kdaImprovement > 0.3) improvements.push(`KDA improved by ${(kdaImprovement * 100).toFixed(1)}%`);
    if (visionImprovement > 0.3) improvements.push(`Vision score improved by ${(visionImprovement * 100).toFixed(1)}%`);
    if (championPoolChange > 0.5) improvements.push('Champion pool changed significantly');
  ...
  private analyzeChampionEvolution(matches: any[]): ChampionEvolution[] {
    // Group matches by champion
    const championMatches = new Map<string, any[]>();
    ...
    const evolutions: ChampionEvolution[] = [];

    championMatches.forEach((championGames, championName) => {
      if (championGames.length < 3) return; // Need at least 3 games for analysis

      // Sort by date
      championGames.sort((a, b) => 
        new Date(a.info.gameCreation).getTime() - new Date(b.info.gameCreation).getTime()
      );
  ...
  private analyzeChampionProgression(championName: string, games: any[]): ChampionEvolution {
    const timeWindows = [];
  ...
  private calculateChampionWindowStats(games: any[]): {
    winRate: number;
    averageKDA: number;
    averageCS: number;
  } {
    if (games.length === 0) return { winRate: 0, averageKDA: 0, averageCS: 0 };
  ...
  private calculateSkillProgression(early: any, late: any): number {
  ...
  private isMetaChampion(championName: string): boolean {
  ...
  private isComplexChampion(championName: string): boolean {
  ...
  private calculateOverallSuspicion(
    ...
    evolutions.forEach(evolution => {
      ...
      if (evolution.suspicionFlags.tooGoodTooFast) championSuspicion += 30;
      if (evolution.suspicionFlags.suddenExpertise) championSuspicion += 25;
      if (evolution.suspicionFlags.metaShift) championSuspicion += 15;
      if (evolution.suspicionFlags.complexityJump) championSuspicion += 20;
    ...
    if (windows.length > 2) {
      ...
      const isMonotonicallyIncreasing = performanceProgression.every((val, i) => 
        i === 0 || val >= performanceProgression[i - 1]
      );
